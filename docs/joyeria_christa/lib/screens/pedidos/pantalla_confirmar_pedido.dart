...
class PantallaConfirmarPedido extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Lee todo como dynamic y conviértelo a String solo para mostrar.
    ...
    Future<void> _verOImprimirFactura() async {
      try {
        ...
        _toast(context, 'No se pudo generar/imprimir el PDF: $e');
      }
    }
    ...
    Future<void> _compartirFactura() async {
      try {
        ...
        await Printing.sharePdf(bytes: bytes, filename: 'factura.pdf');
      } catch (e) {
      ...
      try {
        final pedidoId = await PedidoService().crearPedido(
          ...
          datosEntrega: {
            ...
            'nitDpi': nit,
            'metodoPago': metodoPago,
            if (tarjeta.isNotEmpty) 'tarjeta': tarjeta,
          },
        );
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: _verOImprimirFactura,
  ...
  // 🔧 Ahora acepta dynamic y lo convierte a String para evitar crashes.
